...
st.write("Click **start recording**, then **stop**, then the prescription will be generated.")
...
if "user_notes" not in st.session_state:
    st.session_state.user_notes = ""  # Empty initial value for the text area

# When recording is done, populate the Additional Notes text area
if len(audio) > 0:
    ...
    # Store the pretty_json in session state to persist it
    st.session_state.user_notes = pretty_json


# Display the text area with the updated notes from session state
st.text_area("Generated Prescription (editable)", st.session_state.user_notes, height=200)

# Add a final button below the text area to call the test function
if st.button("Generate PDF Prescription"):
    # Generate the prescription PDF (this should be created in the server by your existing function)
    pdf_file = "prescription.pdf"  # Static path of the file generated by your function
    
    # Assuming the file is already generated and saved on the server
    # You can call the existing `generate_prescription_pdf()` here if needed.
    generate_prescription_pdf()  # This will create 'prescription.pdf' on the server

    # Provide the option to download the generated PDF file
